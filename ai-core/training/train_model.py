"""
SolasGPT unified training script.
Trains a small transformer LM on specialized datasets.
Supports: conversation, coding, minecraft, or combined (all).

Usage:
  python train_model.py --dataset conversation
  python train_model.py --dataset coding
  python train_model.py --dataset minecraft  
  python train_model.py --dataset all
  python train_model.py --epochs 200 --dataset conversation
"""
...
LR         = 2e-4
EVAL_EVERY = 50
# Checkpoint saved to models/ directory
CHECKPOINT = os.path.join(os.path.dirname(__file__), "..", "models", "model_checkpoint.pt")
# ────────────────────────────────────────────────
...
def load_dataset(dataset_type: str, base_dir: str) -> str:
    """Load text data from the specified dataset type."""
    dataset_dir = os.path.join(base_dir, '..', 'datasets', dataset_type)
    
    if dataset_type == 'all':
        # Combine all datasets
        all_text = []
        for ds_type in ['conversation', 'coding', 'minecraft']:
            all_text.append(f"\n### {ds_type.upper()} DATASET ###\n")
            all_text.append(load_dataset(ds_type, base_dir))
        return '\n'.join(all_text)
    
    # Load all .txt files in the dataset directory
    text_files = glob.glob(os.path.join(dataset_dir, '*.txt'))
    if not text_files:
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='conversation',
                        choices=['conversation', 'coding', 'minecraft', 'all'],
                        help='Dataset type: conversation, coding, minecraft, or all')
    ...
    text = load_dataset(args.dataset, script_dir)
